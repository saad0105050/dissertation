...
\begin{definition}[$\mathcal{L}_n(\alpha)$]\label{def:dist-L-alpha-praos}
  ...
  \begin{itemize}
    \item The symbols $w_i$s are independent and 
    either $w_i =\A$ or $w_i \in \{1,2,\ldots\}$. 
    \item In addition, for all $i$, $\Pr[w_i = \A] = \alpha$.
    \item Furthermore, $\Pr[w_i = j], j \in \NN$ is defined as follows: 
...
% Notice that the distribution $\mathcal{L}_n$ assigns zero probability 
% on characteristic strings $w$ with an empty slot. 
% Next, consider a distribution on the characteristic strings containing empty slots.

% \begin{definition}[$\mathcal{L}^\perp_n(\alpha, a)$]\label{def:dist-L-star-alpha-praos}
%   Let $\alpha \in (0,1/2)$ and $n \in \NN$. 
%   $\mathcal{L}^*_n(\alpha)$ is a distribution on characteristic strings 
%   $w = w_1 \ldots w_n$ so that
%   \begin{itemize}
%     \item The symbols $w_i$s are independent. 
%     \item With probability $a$, $w_i \sim \mathcal{L}_1(\alpha)$;
%     $w_i = \perp$ with probability $1 - a$.
%   \end{itemize}
% \end{definition}
...
% Similarly, define 
% \begin{align}
%   g_\perp(n,\epsilon, a) &= g(W) \qquad \text{where} \quad W \sim \mathcal{L}^\perp_n((1-\epsilon)/2, a)\label{eq:praos-gneps-perp}
% \end{align}
