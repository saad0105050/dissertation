...
Our bounds on $g(n,\epsilon)$ 
follow from our estimates of its first and second moments.
Let $\epsilon \in (0,1), \alpha = (1-\epsilon)/2, n \in \NN$, and $W \sim \mathcal{L}_n(\alpha)$. 
In Figure~\ref{fig:praos-gp-moments}, 
we have plotted various moment bounds on the grinding power $g(n,\epsilon)$; 
...
\section{Praos beacon over a single epoch}
...
\begin{theorem}[Praos beacon, single epoch]\label{thm:minentropy-loss-praos-single-epoch}  
  ...
  Let 
  $$
    m \defeq 0.4 (1+2\epsilon) s^2 V(\epsilon)
    \,.
  $$
  Then $$
    g(n,\epsilon) \leq \sqrt{m/\EpsP} \quad \text{except with probability}\quad 2 \EpsP\,.
  $$ 
  In particular, 
  except with probability $2 \EpsP$,
  $g(n, \epsilon) = \Poly(s) = O(k)$ for $\epsilon > 0.809$. 
  As a consequence,
  except with probability $2 \EpsP$,
  the min-entropy loss in 
  the beacon output, $\rho$, is at most
  $$
    (1/2) \log_2 (m/\EpsP) = O(s)
  $$ after an epoch 
  and, in particular, it is $O(\log_2 s)$ for $\epsilon > 0.809$.
\end{theorem}

The proof of Theorem~\ref{thm:minentropy-loss-praos-single-epoch} 
...
\begin{theorem}[Praos beacon over multiple epochs]\label{thm:minentropy-loss-praos-muliti-epochs}  
  Let $\epsilon \in (0.6, 1)$. 
  Recall $\phi(\epsilon)$ from~\eqref{eq:phi_eps}.
  Suppose Praos 
  remains $(\EpsP, k, s)$-secure 
  inside any epoch 
  ...
  Then 
  the min-entropy loss in 
  the beacon output after $L$ epochs, $\rho$, is at most 
  $$
    (1/2) \log_2 (m/\EpsP) = O(s)
  $$
...
The proof of Theorem~\ref{thm:minentropy-loss-praos-muliti-epochs} 
is presented in Section~\ref{sec:proof-praos-theorem}. 
