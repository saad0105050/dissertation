...
it is not hard to verify, by retracing the proof of Claim~\ref{claim:multiple-honest-blocks}, 
that $A(\epsilon,1) = O(1)$.
Define
\begin{equation}\label{eq:praos-func-zeta}
  \zeta(\epsilon) = (1-\epsilon)/2 + \sqrt{1-\epsilon^2}
  \,.
\end{equation}

\begin{proposition}\label{prop:praos-gp-mean}
  Let $n \in \NN$ and $\epsilon \in (0,1)$. 
  Let $W \sim \DistL_n((1-\epsilon)/2)$. 
  Suppose that an execution of Praos satisfies $\sECQ$ property throughout 
  the sequence of slots represented by $W$.
  Then $\Exp g(W) = O(s^2 \zeta(\epsilon)^s )$.
\end{proposition}
