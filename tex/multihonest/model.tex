...
model is handled in Section~\ref{sec:async}.
...
\begin{definition}[Fork]\label{def:fork}
  ...
  \begin{enumerate}[label=(F{\arabic*})]
    ...
    \item\label{fork:unique-honest}\label{fork:multiply-honest}
    each index $i\in P$ 
...
$F\vdash w$.  The conditions~\ref{fork:root}--\ref{fork:honest-depth}
...
\subsection{Adversarial attacks on settlement time; the settlement game}\label{sec:game} 
  ...
  Consequently, in Definition~\ref{def:settlement-insecurity}, 
  ...
  \begin{definition}[Settlement insecurity]\label{def:settlement-insecurity}
  ...
  We present the proof in Section~\ref{sec:bounds-main-proofs}. 
...
%     the adversary delivers these blockchains
  ...
  \begin{theorem}[Main theorem; consistent tie-breaking]\label{thm:main-bivalent}
  ...
  The proof is deferred to Section~\ref{sec:bounds-main-proofs}.
  ...
  In contrast, Theorem~\ref{thm:main} requires 
