...
Praos guarantees liveness (equivalent to $\ECQ$) 
with parameter $s = 8 k/(1+\epsilon)$. 
Let $\Blockchain$ be the above incarnation of Praos 
and let $\gamma$ be the maximum grinding power in an epoch.
Then, by~\cite[Corollary 3]{Praos}, 
$\Blockchain$ is $(\EpsP, k, s)$-secure over $L$ epochs where
$\EpsP = \gamma R L e^{-\Omega(k) } = O(\gamma R L e^{- k/2 \cdot \epsilon^3 (1 - O(\epsilon))  } )$
