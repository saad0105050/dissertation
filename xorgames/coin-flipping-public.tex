...
In this section, we treat to specific incarnations of $\Distribution$, 
...
\section{The Poisson game}\label{sec:poisson}
  ...
  \begin{definition}[Poisson binomial distribution]\label{def:poisson-binomial-distribution}
    ...
    $\sigma = (\sigma_1, \ldots, \sigma_m), \sigma_i \in (0, 1)$, and . 
    ...
    Then $\Distribution$ is called the \emph{Poisson binomial distribution}.
  \end{definition}
  ...
  $$
  LeCam's theorem (below) states that $\Distribution$ is ``statistically close'' to $\Poisson(\alpha)$.
  \begin{theorem*}[LeCam 1960]
  $$
    \sum_k \left\vert \Pr[S = k] - \frac{\alpha^k k^\alpha}{k!} \right\vert < \sum_{k=1}^n \sigma_k^2
  $$
  \end{theorem*}
  This is why $\Distribution$ is sometimes called the \emph{Poisson binomial distribution}. 
