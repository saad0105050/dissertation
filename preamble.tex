...
\tikzmath{
    ...
    function PraosMinEntropyLossMultiEpoch (\e, \k, \L) {
        if (\e >= 0.6) then {
            if (\e >= 0.809) then {
                return 0 ;
            }
            else {
            	return 0.5 * (LnPraosV(\e, \k) - LnPraosEpsP(\e, \k)) / ln(2) ;
            };
        } else {
            return 0 ;
        };
    ...
    function LnPraosPrBadMultiEpoch (\e, \k, \L) {
        if (\e >= 0.809) then {
            return ln(\L) + 0.5 * ( LnPraosV(\e, \k) + LnPraosEpsP(\e, \k) ) ;
        } else {
            return 0 ;
        };
    };
    function PraosPrBadMultiEpoch (\e, \k, \L) {
        if (\e >= 0.6) then {
            \epsp = PraosEpsP(\e, \k) ;
            % return \L * ( sqrt( PraosV(\e) * \epsp ) + 2*\epsp ) ;
            return \L * ( sqrt( PraosV(\e, \k) * \epsp ) ) ;
        } else {
            return 1 ;
        };
    };
