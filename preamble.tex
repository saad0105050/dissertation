...
\newcommand{\ParagraphBf}[1]{\vspace{1em}\noindent\textbf{#1}\ }
...
\tikzmath{
    ...
    function PraosMinEntropyLossMultiEpoch (\e, \k) {
        ...
        \actual = 0.5 * (LnPraosV(\e, \k) - \lnepsp) / ln(2) ;
        return max(\actual, 0) ; % Can't be negative
    };
    function PraosPrBadMultiEpoch (\e, \k, \L) {
        % Probability cannot be larget than 1
        \epsp = min(1, PraosEpsP(\e, \k) );
        \epsp = max(0, \epsp) ;
        if (\epsp < 1) then {
            \Rho = PraosMinEntropyLossMultiEpoch(\e, \k) ;
            return 3 * \L * 2^\Rho \epsp ;
        } else 
        {
            return 1 ;
        };
    ...
    function LnPraosPrBadMultiEpoch (\e, \k, \L) {
        ...
        if (\epsp < 1) then {
            \Rho = PraosMinEntropyLossMultiEpoch(\e, \k) ;
            return ln(3) + ln(\L) + \Rho * ln(2) + ln(\epsp);
        } else {
            return 0 ;
        };
    };
    % XOR target games: Poisson
    function PoissonBeaconMinEntropyLoss (\e, \k, \epsp) {
        \common = 0 ;
        \extra = 0 ;
        if (\epsp < 1) then {
            \a = Alpha(\e) ;
    ...
    function PoissonBeaconPrBadMultiEpoch (\e, \k, \L, \epsp) {
        if (\epsp < 1) then {
            \Rho = PraosMinEntropyLossMultiEpoch(\e, \k) ;
            return 3 * \L * 2^\Rho * \epsp ;
        } 
    ...
    function LnPoissonBeaconPrBadMultiEpoch (\e, \k, \L, \epsp) {
        if (\epsp < 1) then {
            \Rho = PraosMinEntropyLossMultiEpoch(\e, \k) ;
            return ln(3) + ln(\L) + \Rho * ln(2) + ln(\epsp);
        } else {
    ...
    function BernoulliBeaconMinEntropyLoss (\e, \k, \epsp) {
        if (\epsp < 1) then {
            \a = Alpha(\e) ;
            \f = ln(2) / ln(1/\a) ;
            \T = PraosN(\e, \k);
            ...
            \d = max(1,\dmax) ; 
            \Tkd = (\T-\k)/\d ;
            return \k/\d + 1 + \f * ln( (\Tkd/\epsp) / (1+\a) )/ln(2) ;
        } else {
            return -1;
        };
    };
