  ...
  % \subsection{Viable tines, reach, and $\Aheavy$ intervals}
    ...
    % In what follows, we record some useful observations about forks that highlight 
    % the interplay among reach, viability, $\A$-heaviness, and adversarial extensions.
    % % We stress that these observations do not require the honest players 
    % % to use a consistent tie-breaking rule such as $\LCR(\cdot)$.
    ...
    %     % \item \label{fact-part:nonneg-reach} $\reach_{F_x}(B) \geq 0$;
    %     \item \label{fact-part:Aheavy} $I$ is $\Aheavy$; and 
    %     \item \label{fact-part:viable-adv-ext} 
    ...
    % \begin{proof}
    %   First let us prove that~\ref{fact-part:Aheavy} implies~\ref{fact-part:viable-adv-ext}.
    %   Let $t^*$ be a maximum-length honest tine in $F$ so that $\ell(t^*) \in I$. 
    %   There can be two cases.
    %   If $B$ is on $t^*$, 
    %   the adversarial slots in $I$ can be used to create 
    %   an adversarial tine $t$ so that 
    %   i) $B$ is the last honest vertex on $t$, 
    %   ii) $B$ is the last common vertex between $t$ and $t^*$, and 
    %   iii) $\length(t) \geq \length(t^*)$ so that 
    %   $t$ is viable at the onset of slot $s$. 
    %   Now suppose $B$ is not on $t^*$.
    %   Let $B^*$ be the first honest vertex on $t^*$ 
    %   so that $\ell(B^*) \leq \ell(B)$. 
    %   If the interval $I^\prime = [\ell(B^*) + 1, \ell(B) - 1]$ is non-empty, 
    %   $t^*$ must contain only adversarial vertices in $I^\prime$. 
    %   We can build the adversarial tine $t$ as follows: 
    %   Extend $B^*$ by duplicating the vertices on $t^*$ in the interval $[\ell(B^*) + 1, \ell(B) - 1]$, 
    %   put $B$ on $t$ 
    %   and finally, extend $B$ using only adversarial slots from $I$ so that 
    %   $B^*$ is the last common vertex between $t$ and $t^*$, and 
    %   $\length(t) \geq \length(t^*)$. 
    %   Hence $t$ is viable at the onset of slot $s$. 

    %   It remains to prove that~\ref{fact-part:viable-adv-ext} implies~\ref{fact-part:Aheavy}.
    ...
    %   since the longest tine grows by at least one vertex 
    %   for each honest slot in $I$.
    % \end{proof}
